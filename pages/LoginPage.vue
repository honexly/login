<template>
    ...
    <video ref="videoPlayer" autoplay class="autoplay-video" @ended="onVideoEnded">
      <source src="@/assets/video.mp4" type="video/mp4" />
      Your browser does not support the video tag.
    </video>
    <!-- Skip Button will be hidden with CSS -->
    <button class="btn skip-btn" @click="onVideoEnded">Skip</button>
  </div>
 ...
 export default {
  ...
  methods: {
    ...
    // Show the loading screen and navigate to homepage
    showLoadingScreen() {
      this.isLoading = true;
      setTimeout(() => {
        this.isLoading = false; // Stop loading
        this.$router.push("/home"); // Navigate to the homepage
      }, 100); // Loading delay before navigating
    },
  },
};
  </script>
